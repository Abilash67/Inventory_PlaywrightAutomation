...
test.describe("User Management - Complete Test Suite", () => {
  ...
  test("TC03 - Verify user card displays required information", async () => {
    ...
    await expect(card).toContainText(userData.validUser.name);
    await expect(card).toContainText(userData.validUser.employeeId);
  });
  ...
  test("TC04 - Search user by valid name", async () => {
    ...
    await expect(
      userManagementPage.getUserCard(userData.validUser.name),
    ).toBeVisible();
  });
  ...
  test("TC21 - Verify View user functionality", async ({ page }) => {
    await userManagementPage.viewUser(userData.validUser.name);
  ...
  test("TC23 - Verify Edit button is displayed", async () => {
    const hasEditButton = await userManagementPage.hasEditButton(
  ...
  test("TC24 - Verify Assets button is displayed", async () => {
    const hasAssetsButton = await userManagementPage.hasAssetsButton(
      userData.validUser.name,
    );
  ...
  test("TC25 - Verify Delete button is displayed", async () => {
    const hasDeleteButton = await userManagementPage.hasDeleteButton(
      userData.validUser.name,
    );
  ...
  test("TC30 - Verify rapid search changes", async () => {
    await userManagementPage.searchUser("A");
    await userManagementPage.searchUser("Am");
    await userManagementPage.searchUser("Amr");
    await userManagementPage.searchUser("Amrutha");
  ...
  test("TC31 - Verify changing department filter", async () => {
    ...
    const firstCount = await userManagementPage.getUserCount();

    await userManagementPage.selectDepartment("testing");

    const secondCount = await userManagementPage.getUserCount();
  ...
  test("TC33 - Verify user details are not blank", async () => {
    const details = await userManagementPage.getUserDetails(
      userData.validUser.name,
    );
  ...
  test("TC34 - Verify invalid search returns no users", async () => {
    await userManagementPage.searchUser("INVALID_USER_999999");
  ...
  test("TC37 - Verify search field is enabled", async () => {
    await expect(userManagementPage.searchInput).toBeEnabled();
  });

  // TC38 - DEPARTMENT DROPDOWN
  test("TC38 - Verify department dropdown is visible", async () => {
    await expect(userManagementPage.departmentDropdown).toBeVisible();
  });

  // TC39 - LOCATION DROPDOWN
  test("TC39 - Verify location dropdown is visible", async () => {
    await expect(userManagementPage.locationDropdown).toBeVisible();
  });
});
